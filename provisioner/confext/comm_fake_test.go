// Shared test double for the confext provisioner: the same scripted fake
// packersdk.Communicator used by provisioner/sysext/comm_fake_test.go.
//
// The fake is duplicated per provisioner package (rather than shared via a
// provisioner/testcomm package) so test-only helpers never enter the module
// build and the two provisioner packages stay independent. See the sysext
// file header for the full design rationale.
//
// The fake records every Start command, Upload path, UploadDir destination,
// and Download path in call order. Start is answered from a FIFO response
// table; unscripted Start calls are flagged (assertions fail) but still exit.
// Download serves the canned osRelease bytes (the guest /etc/os-release
// read). The persist-flow tests reuse this fake unchanged.
...
// newBakeComm builds a fake communicator. osRelease is the content served by
// Download("/etc/os-release"); responses are the FIFO Start script.
...
var (
	...
	// packer-confext-9f2a (/var/tmp/packer-<type>-<random>/).
...
// preflightResponses scripts the preflight: command -v, systemctl
// --version, id -u. command is the provisioner's configured command.
func preflightResponses(command string) []scriptedResponse {
	return []scriptedResponse{
		{stdout: "/usr/bin/" + command + "\n"},     // command -v
		{stdout: "systemd 255 (255.4-1ubuntu8)\n"}, // systemctl --version (>= 254 floor)
		{stdout: "0\n"}, // id -u
	}
}
