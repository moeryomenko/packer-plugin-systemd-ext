// Package sysext implements the "sysext" provisioner, which applies
// systemd-sysext extension images to the image under construction.
//
...
const (
	...
	// operations.
	defaultCommand = "systemd-sysext"
	// defaultInstallDir is where extension images land on the guest.
	defaultInstallDir = "/var/lib/extensions"
)
...
func (p *Provisioner) Prepare(raws ...interface{}) error {
	p.config = Config{}
	if err := config.Decode(&p.config, &config.DecodeOpts{PluginType: "sysext"}, raws...); err != nil {
		return err
	}
...
func (p *Provisioner) provisionBake(ctx context.Context, ui packersdk.Ui, comm packersdk.Communicator) error {
	...
	return p.provisionFlow(
		...
		func(ctx context.Context, ui packersdk.Ui, comm packersdk.Communicator, placed guestops.BakeExtension) error {
			...
			ui.Say(fmt.Sprintf("sysext: baked extension %s", placed.Name))
			return nil
		},
	)
}
...
func (p *Provisioner) provisionPersist(ctx context.Context, ui packersdk.Ui, comm packersdk.Communicator) error {
	...
	return p.provisionFlow(
		...
		func(ctx context.Context, ui packersdk.Ui, comm packersdk.Communicator, placed guestops.BakeExtension) error {
			...
			ui.Say(fmt.Sprintf("sysext: persisted extension %s", placed.Name))
			return nil
		},
	)
}
...
func (p *Provisioner) packageAndPlaceDirectory(
	...
	stagingDir, err := os.MkdirTemp("", "packer-sysext-")
	if err != nil {
		return guestops.BakeExtension{}, fmt.Errorf("sysext provisioner: extension %q: create staging dir: %w", ext.Name, err)
	}
	...
	if ext.ReleaseFile != "" {
		...
		if err != nil {
			return guestops.BakeExtension{}, fmt.Errorf(
				"sysext provisioner: extension %q: read release_file %q: %w",
				ext.Name,
				ext.ReleaseFile,
				err,
			)
	...
	if format != "directory" {
		...
		if err := extpkg.PackageImage(ext.Source, rawPath, ext.Name, extpkg.ImageFormat(format), osRelease, override, extpkg.TypeSysext); err != nil {
			return guestops.BakeExtension{}, fmt.Errorf("sysext provisioner: extension %q: package: %w", ext.Name, err)
		}
		return p.placeRawImage(ctx, comm, typ, ext.Name, rawPath)
	}

	if err := extpkg.PackageDirectory(ext.Source, stagingDir, ext.Name, osRelease, extpkg.TypeSysext, override); err != nil {
		return guestops.BakeExtension{}, fmt.Errorf("sysext provisioner: extension %q: package: %w", ext.Name, err)
	}
...
func (p *Provisioner) placeRawImage(
	...
	if err != nil {
		return guestops.BakeExtension{}, fmt.Errorf("sysext provisioner: extension %q: %w", name, err)
	}
