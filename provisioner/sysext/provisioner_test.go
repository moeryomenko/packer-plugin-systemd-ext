// Contract tests for the sysext provisioner's Config decoding and
...
// the sysext provisioner.
const (
	wantDefaultCommand    = "systemd-sysext"
	wantDefaultInstallDir = "/var/lib/extensions"
)
...
func TestProvisioner_Impl(t *testing.T) {
	...
	if _, ok := raw.(packersdk.Provisioner); !ok {
		t.Fatalf("sysext Provisioner must be a packersdk.Provisioner")
	}
}
...
func TestPrepareRelativeGuestInstallDir(t *testing.T) {
	var p Provisioner
	cfg := minimalConfig(t)
	cfg["guest_install_dir"] = "var/lib/extensions"
	err := p.Prepare(cfg)
	if err == nil {
